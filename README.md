...
- `WarlockSpells.lua` now contains the inlined look-ahead logic (250ms default reaction time), while `WarlockSpells_lookahead.lua` remains as a reference module.


## WarlockSpells.lua status

`WarlockSpells.lua` now includes the integrated core logic for:
- spellID-first aura/cooldown handling,
- safe cooldown floor management for 12.0.5,
- Nightfall + Shard Instability proc checks,
- GCD + reaction-time (250ms) look-ahead,
- target-switch guard handling.
